...
if (!defined('CARDZNETLIB_INCLUDE_PATH') & CardzNetLibUtilsClass::IsElementSet('get', 'info')) 
{
	phpinfo();
	exit;
}
	
include dirname(dirname(__FILE__)).'/include/cardznetlib_admin.php';

if (!class_exists('CardzNetLibTestBaseClass')) 
{
	class CardzNetLibTestBaseClass extends CardzNetLibAdminBaseClass // Define class
	{
		var $isDevTest = false;
		var $isRunning = false;
		...
		static function GetFileSelect($dir, $id, $fileExtns = 'png,jpg', $selectOptBase = 'ticket_template_', $hideExtn = false)
		{
			...
			$controlValue = CardzNetLibUtilsClass::GetHTTPTextElem('post', $id, '');
			...
			foreach ($selectOpts as $selectOptKey => $selectOptText)
			{
				...
				if ($hideExtn)
					$selectOptValue = substr($selectOptText, 0, strlen($selectOptText)-strlen($fileExtn)-1);
				else
	...
	class CardzNetLibDevTestBaseClass extends CardzNetLibTestBaseClass // Define class
	{
		function __construct($env) //constructor	
		{
			$this->isDevTest = true;
