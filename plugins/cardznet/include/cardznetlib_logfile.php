...
if (!class_exists('CardzNetLibLogFileClass')) 
{
	class CardzNetLibLogFileClass // Define class
	{
		const ForReading = 1;
		const ForWriting = 2;
		...
		function __construct($LogsFolderPath = '')
		{
			...
			if (substr($LogsFolderPath, -1) === '/')
			{
				$LogsFolderPath = substr($LogsFolderPath, 0, -1);
			}
			
			if (!is_dir($LogsFolderPath))
			{
				mkdir($LogsFolderPath, CARDZNETLIB_LOGFOLDER_PERMS, true);
			}
		...
		function GetLogFilePath($Filename)
		{
			...
			for ($i = 1; $i <= 2; $i++)
			{
				...
				else if ($i == 1)
				{
					...
					if (strlen($this->LogsFolderPath) > 0)
		...
		function AddToTestLog($LogLine)			
		{
			...
			self::LogToFileAbs($Filepath, 'Log Time/Date:'.CardzNetLibDBaseClass::GetLocalDateTime()."\n");
			//self::LogToFileAbs($Filepath, 'Request URL:'.CardzNetLibUtilsClass::GetHTTPTextElem('server', 'REQUEST_URI')."\n");
